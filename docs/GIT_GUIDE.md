...
代码、仓库内 docs 和测试由 Git 保存；MuziWorkspace 数据库、课件和运行缓存不由该仓库保存。

改造前提交为 `ad6b3bd`，标签为 `learning-v1-before`；学习第一版使用标签 `learning-v1`。
...
本指南也包含在第一版提交内。撤回后文件会消失，可随时用 `git show learning-v1:docs/GIT_GUIDE.md` 查看。
