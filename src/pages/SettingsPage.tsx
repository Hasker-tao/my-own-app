    ...
    <div>
      ...
      <Section title="使用偏好" description="设置会保存在主数据文件中">
        <div className="preferences">
          <div><div><strong>界面风格</strong><small>功能和数据保持一致，只改变视觉系统</small></div><div className="appearance-toggle style-toggle" role="group" aria-label="界面风格"><button className={appearance === "liquid" ? "active" : ""} aria-label="Liquid Glass" aria-pressed={appearance === "liquid"} onClick={() => void saveSetting("appearance", "liquid")}><span><Stack size={17} /><strong>Liquid Glass</strong></span><small>环境色、透明材质与柔和层次</small></button><button className={appearance === "notebook" ? "active" : ""} aria-label="桌面 App" aria-pressed={appearance === "notebook"} onClick={() => void saveSetting("appearance", "notebook")}><span><Notebook size={17} /><strong>桌面 App</strong></span><small>精致层级、紧凑布局与清晰交互</small></button><button className={appearance === "neo" ? "active" : ""} aria-label="Neo-Brutalism" aria-pressed={appearance === "neo"} onClick={() => void saveSetting("appearance", "neo")}><span><SquaresFour size={17} /><strong>Neo-Brutalism</strong></span><small>多色印刷、硬边框与机械反馈</small></button></div></div>
          <label><div><strong>界面语言</strong><small>仅翻译界面文案；个人记录和课程资料保持原文</small></div><select aria-label="界面语言" value={data.settings.language === "en" ? "en" : "zh-CN"} onChange={(event) => void saveSetting("language", event.target.value)}><option value="zh-CN">简体中文</option><option value="en">English</option></select></label>
