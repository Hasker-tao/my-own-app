...
import { courseProgress, todaySuggestions } from "../features/learning";
...
export function LearningSummary() {
  ...
  return <Section title="我的学习" description="只按真实学习记录更新，不自动累计进度" action={<Link to="/learning">打开学习模块 →</Link>}><p>{courses.length ? `${courses.length} 门本学期课程 · 已授课中已复习 ${covered}/${taught} 个主题 · 今日 ${count} 项可选复习建议` : "从 Moodle 导入本学期课程，开始记录章节进度。"}</p>{data.sync.status==="error"&&<p className="quiet-line">{data.sync.message}</p>}</Section>;
}
