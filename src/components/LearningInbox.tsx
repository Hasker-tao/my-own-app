...
import { courseworkTasks, type LearningState } from "../features/learning";

const statuses = { unknown: "待核实", not_submitted: "未提交", draft: "已上传草稿 · 尚未提交", submitted: "已正式提交" };
export function LearningInbox({ state, view, busy, onRead, onSync }: { state: LearningState; view: "tasks" | "news"; busy: boolean; onRead: (ids: string[]) => void; onSync: () => void }) {
  ...
  const tasks = courseworkTasks(state).filter(t => showSubmitted || t.resource.coursework?.status !== "submitted" || t.resource.coursework?.error || t.resource.missing);
  return <Section title="Coursework 待办" description="未提交、草稿和待核实的作业集中放在这里。上传文件不等于正式提交，最终状态以学校页面为准。" action={<Button variant="secondary" loading={busy} onClick={onSync}>同步作业状态</Button>}>
    ...
    {tasks.length ? tasks.map(({course,resource}) => { const detail = resource.coursework; const uncertain = !detail || detail.error || resource.missing; return <article className="learning-coursework" key={`${course.id}:${resource.id}`}><div><small>{course.title}</small><h3>{resource.title}</h3><Badge tone={uncertain || detail?.status !== "submitted" ? "warning" : "success"}>{uncertain ? "待核实" : statuses[detail.status]}</Badge></div>
